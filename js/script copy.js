...
//   document.getElementById("genderSelectionModal")
// );
// myModal.show();
...
pokemons.forEach((pokemon) => {
  ...
  card.innerHTML = `
    ...
    <p>${pokemon.name}</p>
    <p class="text-muted">Type: ${pokemon.type.join("/")}</p>
    <p class="text-muted">Ability: ${pokemon.ability}</p>
    <div class="row">
      <div class="col">
        <p class="text-muted">Move: ${pokemon.moves
          .slice(0, 2)
          .join("</p><p class='text-muted'>Move: ")}</p>
      </div>
      <div class="col">
        <p class="text-muted">Move: ${pokemon.moves
          .slice(2)
